...
3. Агент проверяет текущую нагрузку сервера: CPU, свободную RAM и наличие других build-процессов.
4. Если сервер перегружен, сборка откладывается до следующего прохода scheduled task.
5. Если сервер свободен, агент использует модели из `C:\ProgramData\NeuroWingsBuilder\models-source`.
6. Собирается новый `EXE`.
7. Релиз публикуется на Linux server.
8. Вместе с релизом публикуется документация.
9. `update-feed.json` обновляется до новой версии.
...
- Перед тяжелой сборкой агент теперь проверяет нагрузку и может отложить релиз, если хост занят.
- Агент также проверяет свободное место на системном диске перед onefile-сборкой.
